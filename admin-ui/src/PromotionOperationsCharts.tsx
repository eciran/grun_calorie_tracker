...
function horizontalBarOption(
  ...
  return {
    graphic: visible.length ? undefined : {
      ...
      style: { text: noDataText, fill: palette.muted, fontSize: 13 }
    },
...
export function PromotionLifecycleChart({ analytics }: { analytics: AdminPromotionOperationsAnalytics }) {
  ...
  const buildOption = useCallback(() => horizontalBarOption(
    ...
    { ACTIVE: "#2f8f68", DRAFT: "#8065a0", DEACTIVATED: "#7b7f88", EXPIRED: "#b7832f" }, locale === "tr" ? "Henüz veri yok" : "No data yet"
  ), [locale, metrics]);
  return <AdminEChart ariaLabel={locale === "tr" ? "Promosyon yaşam döngüsü grafiği" : "Promotion lifecycle chart"} buildOption={buildOption} className="promotion-operations-echart" />;
}
...
export function PromotionTypeChart({ analytics }: { analytics: AdminPromotionOperationsAnalytics }) {
  ...
  const buildOption = useCallback(() => horizontalBarOption(
    ...
    { CAMPAIGN: "#8065a0", INTRO_OFFER: "#397ea8", WIN_BACK: "#b7832f", SUPPORT_GRANT: "#2f8f68" }, locale === "tr" ? "Henüz veri yok" : "No data yet"
  ), [locale, metrics]);
...
export function RedemptionOutcomeChart({ analytics }: { analytics: AdminPromotionOperationsAnalytics }) {
  ...
  const buildOption = useCallback(() => horizontalBarOption(
    ...
    { CONVERTED: "#2f8f68", REJECTED: "#b63c4b", PROVIDER_VERIFIED: "#397ea8", RESERVED: "#b7832f" }, locale === "tr" ? "Henüz veri yok" : "No data yet"
  ), [locale, metrics]);
...
export function PromotionRejectionChart({ analytics }: { analytics: AdminPromotionOperationsAnalytics }) {
  ...
  const buildOption = useCallback(() => horizontalBarOption(
    ...
    { LIMIT: "#b7832f", ELIGIBILITY: "#8065a0", PROVIDER: "#b63c4b", OTHER: "#7b7f88" }, locale === "tr" ? "Henüz veri yok" : "No data yet"
  ), [locale, metrics]);
  ...
  const buildOption = useCallback((): EChartsCoreOption => {
    ...
    return {
      graphic: hasActivity ? undefined : {
        type: "text", left: "center", top: "middle",
        style: { text: locale === "tr" ? "Bu aralıkta kullanım etkinliği yok" : "No redemption activity in this window", fill: palette.muted, fontSize: 13 }
      },
      ...
      series: [
        {
          type: "bar", name: locale === "tr" ? "Denemeler" : "Attempts", data: points.map((item) => Number(item.attempts ?? 0)),
          itemStyle: { color: "rgba(115, 78, 150, 0.28)", borderRadius: [3, 3, 0, 0] }, barMaxWidth: 24
        },
        {
          type: "line", name: locale === "tr" ? "Dönüşen" : "Converted", data: points.map((item) => Number(item.converted ?? 0)),
          smooth: true, symbolSize: 6, lineStyle: { width: 3, color: "#2f8f68" }, itemStyle: { color: "#2f8f68" }
        },
        {
          type: "line", name: locale === "tr" ? "Reddedilen" : "Rejected", data: points.map((item) => Number(item.rejected ?? 0)),
          smooth: true, symbolSize: 6, lineStyle: { width: 2, color: "#b63c4b" }, itemStyle: { color: "#b63c4b" }
        },
        {
          type: "line", name: locale === "tr" ? "Tekrarlar" : "Duplicates", data: points.map((item) => Number(item.duplicateAttempts ?? 0)),
          smooth: true, symbolSize: 5, lineStyle: { width: 2, type: "dashed", color: "#b7832f" }, itemStyle: { color: "#b7832f" }
        }
      ]
    };
  }, [analytics.windowDays, locale, points]);
