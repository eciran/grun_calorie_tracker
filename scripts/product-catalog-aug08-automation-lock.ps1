...
$lockDirectory = Join-Path $workspace "tmp\product-catalog-aug08"
...
function Read-Lock {
    ...
    try {
        ...
        return [pscustomobject]@{ runId = "unreadable"; startedAt = $null; branch = $null }
    }
}
...
if ($Action -eq "Status") {
    ...
    Write-Result @{
        status = "LOCKED"; lockPath = $lockPath; runId = $existing.runId
        startedAt = $existing.startedAt; branch = $existing.branch
    }
    exit 0
}
...
if ($Action -eq "Release") {
    ...
    if ($existing.runId -ne $RunId) {
        Write-Result @{ status = "NOT_OWNER"; requestedRunId = $RunId; ownerRunId = $existing.runId }
        exit 4
    }
...
if ($null -ne $existing) {
    ...
    if ($existing.startedAt) {
        try { $startedAt = [DateTimeOffset]::Parse($existing.startedAt) } catch { $startedAt = $null }
    }
    $isStale = $null -eq $startedAt -or $startedAt -lt [DateTimeOffset]::UtcNow.AddMinutes(-$StaleAfterMinutes)
    if (-not $isStale) {
        Write-Result @{
            status = "BUSY"; ownerRunId = $existing.runId
            startedAt = $existing.startedAt; branch = $existing.branch
        }
        exit 3
    }
...
Write-Result @{ status = "ACQUIRED"; runId = $RunId; branch = $branch; lockPath = $lockPath }
