...
public class AdminInvitationServiceImpl implements AdminInvitationService {
 ...
 private AdminInvitationDto issue(UserEntity owner,String email,UserRole role,AdminAuditActionType action,String cid){String raw=token();AdminInvitationEntity i=new AdminInvitationEntity();i.setEmail(email);i.setRole(role);i.setTokenHash(hash(raw));i.setStatus(AdminInvitationStatus.PENDING);i.setInvitedBy(owner);i.setExpiresAt(Instant.now().plus(Duration.ofHours(Math.max(1,expirationHours))));AdminInvitationEntity saved=invitationRepository.save(i);String link=baseUrl+(baseUrl.contains("?")?"&":"?")+"token="+raw;String subject="Your GRUN admin invitation";String text="You were invited to the GRUN admin team as "+role.name()+". Activate your account: "+link;String html="<p>You were invited to the GRUN admin team as <strong>"+role.name()+"</strong>.</p><p><a href=\""+link+"\">Activate account and set password</a></p><p>This single-use link expires in "+expirationHours+" hours.</p>";if(mailProperties==null){mailDeliveryService.sendTransactionalEmail(email,subject,text,html);}else{boolean tr=owner.getPreferredLanguage()==PreferredLanguage.TR;long templateId=tr?mailProperties.getBrevo().getTemplates().getAdminInvitationTr():mailProperties.getBrevo().getTemplates().getAdminInvitationEn();mailDeliveryService.sendTransactionalTemplate(email,templateId,Map.of("invitationUrl",link,"role",role.name(),"expirationHours",expirationHours),subject,text,html);}audit(owner.getEmail(),action,saved,cid);return dto(saved);}
