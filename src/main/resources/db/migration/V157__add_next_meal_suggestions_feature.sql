...
ALTER TABLE subscription_plan_features
    ...
    CHECK (feature IN (
        ...
        'MICRONUTRIENT_DETAILS','DATA_EXPORT','FASTING_BASIC','FASTING_ADVANCED',
...
VALUES
    ('FREE', 'NEXT_MEAL_SUGGESTIONS', FALSE, CURRENT_DATE, CURRENT_TIMESTAMP),
    ('PLUS', 'NEXT_MEAL_SUGGESTIONS', TRUE, CURRENT_DATE, CURRENT_TIMESTAMP),
    ('PRO', 'NEXT_MEAL_SUGGESTIONS', TRUE, CURRENT_DATE, CURRENT_TIMESTAMP)
...
WHERE subscription.plan_type IN ('PLUS', 'PRO')
  ...
  AND NOT EXISTS (
      ...
      WHERE entitlement.subscription_id = subscription.id
        AND entitlement.feature = 'NEXT_MEAL_SUGGESTIONS'
