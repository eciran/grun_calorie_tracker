...
class AdminMealReminderAutomationServiceImplTest {
 ...
 MealReminderCandidateWorker worker=mock(MealReminderCandidateWorker.class); MealReminderDeliveryProperties deployment=new MealReminderDeliveryProperties(); PushProperties push=new PushProperties(); AdminAuditService audit=mock(AdminAuditService.class);
 Instant now=Instant.parse("2026-08-29T12:00:00Z"); AdminMealReminderAutomationServiceImpl service;
 @BeforeEach void setUp(){service=new AdminMealReminderAutomationServiceImpl(policies,decisions,occurrences,interactions,attempts,outboxes,sends,users,definitions,worker,deployment,push,audit,Clock.fixed(now,ZoneOffset.UTC));}
