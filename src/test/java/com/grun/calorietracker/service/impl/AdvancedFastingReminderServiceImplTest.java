...
class AdvancedFastingReminderServiceImplTest {
 ...
 @Test void legacyQuietHoursDoNotDeferDueReminder(){ user.setNotificationQuietHoursStart(LocalTime.of(22,0)); user.setNotificationQuietHoursEnd(LocalTime.of(7,0)); now=LocalDateTime.of(2026,7,28,23,0); occurrence.setPlannedStartAt(now.plusMinutes(30)); when(deliveries.findByOccurrenceKey(key())).thenReturn(Optional.empty()); when(deliveries.save(any())).thenAnswer(i->i.getArgument(0)); when(notifications.save(any())).thenAnswer(i->i.getArgument(0)); when(push.deliver(any())).thenReturn(new PushDeliveryResultDto(1,1,0,0)); assertEquals(1,service.dispatchDue(occurrence,now)); ArgumentCaptor<FastingReminderDeliveryEntity> captor=ArgumentCaptor.forClass(FastingReminderDeliveryEntity.class); verify(deliveries).save(captor.capture()); assertEquals(FastingReminderDeliveryStatus.SENT,captor.getValue().getStatus()); assertNull(captor.getValue().getNextAttemptAt()); }
